...
t_appear=[5 5];% target appearing time
t_disappear=[15 15]; % target disappearing time
...
% ---------------------- update particle xp(:,k+1,i)
 ...
 if  Pb1(1,k+1)>=0.7  
z1e1=z;%在雷达自己的坐标系下求目标一估计值的三维量测 

z1e1(1,k+1)=sqrt(axe1(1,k+1).^2+(axe1(3,k+1)+100).^2); % z1=range between target and sensor, sensor is located in the origin
z1e1(2,k+1)=(axe1(1,k+1).*axe1(2,k+1)+(axe1(3,k+1)+100).*axe1(4,k+1))./z1e1(1,k+1); % z2=doppler,  note!!!: there is a jump in doppler measure
z1e1(3,k+1)=atan(axe1(1,k+1)./(axe1(3,k+1)+100)); % z3=bearing, from North (positive Y axis) to the trajectory 

% 在雷达自己的坐标系下求目标一真实值的三维量测

% z1e11(1,k+1)=sqrt(ax1(1,k+1).^2+(ax1(3,k+1)+100).^2); % z1=range between target and sensor, sensor is located in the origin
% z1e11(2,k+1)=(ax1(1,k+1).*ax1(2,k+1)+(ax1(3,k+1)+100).*ax1(4,k+1))./z1e11(1,k+1); % z2=doppler,  note!!!: there is a jump in doppler measure
% z1e11(3,k+1)=atan(ax1(1,k+1)./(ax1(3,k+1)+100)); % z3=bearing, from North (positive Y axis) to the trajectory 
%   
%根据目标一估计值的三维量测，得到每帧量测 
    for ie=1:Nr % i corresponding to r
        for j=1:Nd % j corresponding to d
            for m=1:Nb % m corresponding to b
                n_I=delta_n*randn;
                n_Q=delta_n*randn;
                h_rdb_e1(ie,j,m,k+1)=exp(-0.5*Lr*(r(ie)-z1e1(1,k+1))^2/Dr-0.5*Ld*(d(j)-z1e1(2,k+1))^2/Dd-0.5*Lb*(b(m)-z1e1(3,k+1))^2/Db);
                z_rdbe1_I(ie,j,m,k+1)=Amp*h_rdb_e1(ie,j,m,k+1)*cos(fai)+n_I;
                z_rdbe1_Q(ie,j,m,k+1)=Amp*h_rdb_e1(ie,j,m,k+1)*sin(fai)+n_Q; 
                z_rdb_e1(ie,j,m,k+1)=z_rdbe1_I(ie,j,m,k+1)^2+z_rdbe1_Q(ie,j,m,k+1)^2;
%               h_rdb_e1(i,j,m,k+1)=exp(-0.5*Lr*(r(i)-z1e1(1,k+1))^2/Dr-0.5*Ld*(d(j)-z1e1(2,k+1))^2/Dd-0.5*Lb*(b(m)-z1e1(3,k+1))^2/Db);
% 
%                  z_rdbe1_I(i,j,m,k+1)=Amp*h_rdb_e1(i,j,m,k+1)*cos(fai)+n_I;
%                 z_rdbe1_Q(i,j,m,k+1)=Amp*h_rdb_e1(i,j,m,k+1)*sin(fai)+n_Q;
% 
%                 z_rdb_e1(i,j,m,k+1)=z_rdbe1_I(i,j,m,k+1)^2+z_rdbe1_Q(i,j,m,k+1)^2;
            end
        end
    end
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %根据目标一真实值的三维量测，得到每帧量测 
%        for ie1=1:Nr % i corresponding to r
%         for j1=1:Nd % j corresponding to d
%             for m1=1:Nb % m corresponding to b
%                 n1_I=delta_n*randn;
%                 n1_Q=delta_n*randn;
%                 h_rdb_e11(ie1,j1,m1,k+1)=exp(-0.5*Lr*(r(ie1)-z1e11(1,k+1))^2/Dr-0.5*Ld*(d(j1)-z1e11(2,k+1))^2/Dd-0.5*Lb*(b(m)-z1e11(3,k+1))^2/Db);
%                 z_rdbe11_I(ie1,j1,m1,k+1)=Amp*h_rdb_e11(ie1,j1,m1,k+1)*cos(fai)+n1_I;
%                 z_rdbe11_Q(ie1,j1,m1,k+1)=Amp*h_rdb_e11(ie1,j1,m1,k+1)*sin(fai)+n1_Q; 
%                 z_rdb_e11(ie1,j1,m1,k+1)=z_rdbe1_I(ie1,j1,m1,k+1)^2+z_rdbe1_Q(ie1,j1,m1,k+1)^2;
% %               h_rdb_e1(i,j,m,k+1)=exp(-0.5*Lr*(r(i)-z1e1(1,k+1))^2/Dr-0.5*Ld*(d(j)-z1e1(2,k+1))^2/Dd-0.5*Lb*(b(m)-z1e1(3,k+1))^2/Db);
% % 
% %                  z_rdbe1_I(i,j,m,k+1)=Amp*h_rdb_e1(i,j,m,k+1)*cos(fai)+n_I;
% %                 z_rdbe1_Q(i,j,m,k+1)=Amp*h_rdb_e1(i,j,m,k+1)*sin(fai)+n_Q;
...
%     z_rdb12(:,:,:,k+1)=z_rdb11(:,:,:,k+1)- z_rdb_e1(:,:,:,k+1)- z_rdb_e11(ie1,j1,m1,k+1);
    z_rdb12(:,:,:,k+1)=z_rdb11(:,:,:,k+1)- z_rdb_e1(:,:,:,k+1);
%z_rdb12(:,:,:,k+1)= z_rdb_e1(:,:,:,k+1);
 
 
 else
     
     
     %设定如果没有检测到目标一，粒子群接受到的量测为噪声
    for iee=1:Nr % i corresponding to r
        for j=1:Nd % j corresponding to d
            for m=1:Nb % m corresponding to b
                n_I=delta_n*randn;
                n_Q=delta_n*randn;
                z_rdb_I(iee,j,m,k+1)=n_I;
                z_rdb_Q(iee,j,m,k+1)=n_Q;
                z_rdb12(iee,j,m,k+1)=z_rdb_I(iee,j,m,k+1)^2+z_rdb_Q(iee,j,m,k+1)^2;% power
            end
        end
    end
