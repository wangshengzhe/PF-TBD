function [z_rdb,h_rdb]=computRealTMulcai(Nr,Nd,Nb,delta_n,Lr,Dr,Ld,Db,Amp,z,d,b,r,Dd,Lb,fai,TE)
%计算总的量测，z相加
%   
[NumTarget, timesensor]=size(TE);%目标个数和仿真时间长度
h_rdb=zeros(Nr,Nd,Nb,timesensor);
z_rdb=zeros(Nr,Nd,Nb,timesensor);
for k=1:timesensor
if TE(:,k)==0%没有雷达有探测信息
                ...
                for p=1:NumTarget
                    if TE(p,k)==1
                       ...
                       % h_rdb(i,j,m,k)=h_rdb(i,j,m,k)+exp(-0.5*Lr*(r(i)-z((p-1)*3+1,k))^2/Dr-0.5*Ld*(d(j)-z((p-1)*3+2,k))^2/Dd-0.5*Lb*(b(m)-z((p-1)*3+3,k))^2/Db);
                         h_rdb(i,j,m,k)=exp(-0.5*Lr*(r(i)-z((p-1)*3+1,k))^2/Dr-0.5*Ld*(d(j)-z((p-1)*3+2,k))^2/Dd-0.5*Lb*(b(m)-z((p-1)*3+3,k))^2/Db);
                z_rdb_I(i,j,m,k)=Amp*h_rdb(i,j,m,k)*cos(fai)+n_I;
                z_rdb_Q(i,j,m,k)=Amp*h_rdb(i,j,m,k)*sin(fai)+n_Q;
              % z_rdb(i,j,m,k)=sqrt(z_rdb_I(i,j,m,k)^2+z_rdb_Q(i,j,m,k)^2);%amplitude
                z_rdb(i,j,m,k)= z_rdb(i,j,m,k)+z_rdb_I(i,j,m,k)^2+z_rdb_Q(i,j,m,k)^2;     
